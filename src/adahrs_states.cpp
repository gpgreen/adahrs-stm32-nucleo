...
void ADAHRSSensorData::copy_config_array_to_states(ADAHRSConfig* config)
{
    ...
    // Gyro biases
    state_data.beta_p = (int16_t)(config->get_register(UM6_GYRO_BIAS_XY) >> 16);
    state_data.beta_q = (int16_t)(config->get_register(UM6_GYRO_BIAS_XY) & 0x0FFFF);
    state_data.beta_r = (int16_t)(config->get_register(UM6_GYRO_BIAS_Z) >> 16);
    ...
    // Accel biases
    state_data.beta_acc_x = (int16_t)(config->get_register(UM6_ACCEL_BIAS_XY) >> 16);
    state_data.beta_acc_y = (int16_t)(config->get_register(UM6_ACCEL_BIAS_XY) & 0x0FFFF);
    state_data.beta_acc_z = (int16_t)(config->get_register(UM6_ACCEL_BIAS_Z) >> 16);
	 
    // Mag biases
    state_data.beta_mag_x = (int16_t)(config->get_register(UM6_MAG_BIAS_XY) >> 16);
    state_data.beta_mag_y = (int16_t)(config->get_register(UM6_MAG_BIAS_XY) & 0x0FFFF);
...
void ADAHRSSensorData::copy_states_to_data(ADAHRSConfig* config)
{
    ...
    config->set_register(UM6_GYRO_RAW_XY, (uint32_t)((raw_data.gyro_x << 16)
                                                     | (raw_data.gyro_y & 0x0FFFF)));
    config->set_register(UM6_GYRO_RAW_Z, (uint32_t)(raw_data.gyro_z << 16));
	 
    config->set_register(UM6_ACCEL_RAW_XY, (uint32_t)((raw_data.accel_x << 16)
                                                      | (raw_data.accel_y & 0x0FFFF)));
    config->set_register(UM6_ACCEL_RAW_Z, (uint32_t)(raw_data.accel_z << 16));
	 
    config->set_register(UM6_MAG_RAW_XY, (uint32_t)((raw_data.mag_x << 16)
                                                    | (raw_data.mag_y & 0x0FFFF)));
    config->set_register(UM6_MAG_RAW_Z, (uint32_t)(raw_data.mag_z << 16));
	 
    // Processed gyros
    // The maximum measureable rotation rate is +/-2000 deg/sec.  We have 16 bits to represent it as a signed integer
    // 2^16/4000 = 16.384
    converted_data1 = (int16_t)roundf(state_data.gyro_x * 16.384f);
    converted_data2 = (int16_t)roundf(state_data.gyro_y * 16.384f);
    config->set_register(UM6_GYRO_PROC_XY, (uint32_t)((converted_data1 << 16)
                                                      | (converted_data2 & 0x0FFFF)));
	 
    converted_data1 = (int16_t)roundf(state_data.gyro_z * 16.384f);
    config->set_register(UM6_GYRO_PROC_Z, (uint32_t)(converted_data1 << 16));
    ...
    converted_data1 = (int16_t)roundf(state_data.accel_x * 5461.33333333f);
    converted_data2 = (int16_t)roundf(state_data.accel_y * 5461.33333333f);
    config->set_register(UM6_ACCEL_PROC_XY, (uint32_t)((converted_data1 << 16)
                                                       | (converted_data2 & 0x0FFFF)));
	 
    converted_data1 = (int16_t)roundf(state_data.accel_z * 5461.33333333f);
    config->set_register(UM6_ACCEL_PROC_Z, (uint32_t)(converted_data1 << 16));	
    ...
    // 2^16/20 = 3276.8
    converted_data1 = (int16_t)roundf(state_data.mag_x * 3276.8f);
    converted_data2 = (int16_t)roundf(state_data.mag_y * 3276.8f);
    config->set_register(UM6_MAG_PROC_XY, (uint32_t)((converted_data1 << 16)
                                                     | (converted_data2 & 0x0FFFF)));
	 
    converted_data1 = (int16_t)roundf(state_data.mag_z * 3276.8f);
    config->set_register(UM6_MAG_PROC_Z, (uint32_t)(converted_data1 << 16));
	 
    // Euler angles
    // Maximum euler angle values are +/360 degrees.
    // 2^16/(360*2) = 91.0222222222
    converted_data1 = (int16_t)roundf(state_data.phi * 91.0222222f);
    converted_data2 = (int16_t)roundf(state_data.theta * 91.0222222f);
    config->set_register(UM6_EULER_PHI_THETA, (uint32_t)((converted_data1 << 16)
                                                         | (converted_data2 & 0x0FFFF)));
	 
    converted_data1 = (int16_t)roundf(state_data.psi * 91.0222222f);
    config->set_register(UM6_EULER_PSI, (uint32_t)(converted_data1 << 16));
    ...
    uint32_t course_speed = (uint16_t)(roundf(raw_data.GPS_course * 10)) << 16;
    course_speed |= (uint16_t)(roundf(state_data.GPS_speed * 100));
    ...
    uint16_t uint_data1 = (uint16_t)(raw_data.GPS_Hdop*10);
    uint16_t uint_data2 = (uint16_t)(raw_data.GPS_Vdop*10);
    ...
    config->set_register(UM6_GPS_SAT_1_2, (raw_data.GPS_SatID_1 << 24)
                         | (raw_data.GPS_SatSNR_1 << 16)
                         | (raw_data.GPS_SatID_2 << 8)
                         | (raw_data.GPS_SatSNR_2));
    config->set_register(UM6_GPS_SAT_3_4, (raw_data.GPS_SatID_3 << 24)
                         | (raw_data.GPS_SatSNR_3 << 16)
                         | (raw_data.GPS_SatID_4 << 8)
                         | (raw_data.GPS_SatSNR_4));
    config->set_register(UM6_GPS_SAT_5_6, (raw_data.GPS_SatID_5 << 24)
                         | (raw_data.GPS_SatSNR_5 << 16)
                         | (raw_data.GPS_SatID_6 << 8)
                         | (raw_data.GPS_SatSNR_6));
    config->set_register(UM6_GPS_SAT_7_8, (raw_data.GPS_SatID_7 << 24)
                         | (raw_data.GPS_SatSNR_7 << 16)
                         | (raw_data.GPS_SatID_8 << 8)
                         | (raw_data.GPS_SatSNR_8));
    config->set_register(UM6_GPS_SAT_9_10, (raw_data.GPS_SatID_9 << 24)
                         | (raw_data.GPS_SatSNR_9 << 16)
                         | (raw_data.GPS_SatID_10 << 8)
                         | (raw_data.GPS_SatSNR_10));
    config->set_register(UM6_GPS_SAT_11_12, (raw_data.GPS_SatID_11 << 24)
                         | (raw_data.GPS_SatSNR_11 << 16)
                         | (raw_data.GPS_SatID_12 << 8)
