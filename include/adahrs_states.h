...
#ifndef ADAHRS_STATES_H_
#define ADAHRS_STATES_H_
...
typedef struct __Raw_sensor_data
{
    int16_t gyro_x;
    int16_t gyro_y;
    int16_t gyro_z;
...
typedef struct __ADAHRS_state_data {
...
class ADAHRSSensorData
{
public:
    enum EKF_mode {MODE_QUATERNION=0, MODE_EULER};
    
    // data is public, not trying to protect from outside
    RawSensorData raw_data;
    ADAHRS_state_data state_data;

    explicit ADAHRSSensorData();
...
private:
    ...
    const ADAHRSSensorData& operator=(const ADAHRSSensorData&);
